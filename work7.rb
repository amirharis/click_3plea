...
loop do
	...
	@process = State.where(:click_date => Time.now.strftime("%Y-%m-%d"), :process => "7").first
	@process = State.create(:click_date => Time.now.strftime("%Y-%m-%d"), :process => "7") if @process.nil?
	puts "asdas"
