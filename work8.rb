...
loop do
	...
	@process = State.where(:click_date => Time.now.strftime("%Y-%m-%d"), :process => "8").first
	@process = State.create(:click_date => Time.now.strftime("%Y-%m-%d"), :process => "8") if @process.nil?
	puts "asdas"
